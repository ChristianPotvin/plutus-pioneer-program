...
mkPolicy pkh deadline ctx = traceIfFalse "wrong signature"     hasTxBeenSigned   &&
                            traceIfFalse "deadline has passed" isTxDeadlineValid
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    hasTxBeenSigned :: Bool
    hasTxBeenSigned = txSignedBy (scriptContextTxInfo ctx) pkh

    isTxDeadlineValid :: Bool
    isTxDeadlineValid = to deadline `contains` txInfoValidRange info
